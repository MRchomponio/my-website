...
export function MarketplaceFilters({ games }: { games: Game[] }) {
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {games.map((game) => (
          <button
            ...
            className={cn(
              "px-3 py-1.5 rounded-full text-sm border transition-colors",
              activeGame === game.slug ? "text-white" : "border-background-border text-foreground-muted hover:text-foreground"
            )}
            style={
